...
**Current: M3 — `reference-model/`**. M2 is complete: `crates/futarchy-fixed` now uses deterministic integer/table range reduction and fixed-point iteration for `exp2`, `log2`, `ln`, stable two-outcome LMSR cost, marginal price/displacement helpers, buy/sell delta helpers, two-limb checked arithmetic, maker-adverse rounding helpers, and generated high-precision LMSR/transcendental regression corpora with freshness checks. Next is the independent M3 Python reference model and release-scale MPFR-256 corpus/vector regeneration gate.
