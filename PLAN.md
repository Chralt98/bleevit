...
**Current: M2 — `crates/futarchy-fixed`**. The 64.64 LMSR math crate now builds its transcendental surface in `no_std` mode with deterministic in-crate `f64` range reduction/series helpers, checked arithmetic, maker-adverse rounding helpers, stable two-outcome LMSR cost, and regression coverage for V1/V4; remaining work is replacing this interim deterministic kernel with the spec-required ≤2 ulp polynomial/integer implementation and adding the full MPFR-256 corpus gate before M2 can be marked complete.
...
| M2 | `crates/futarchy-fixed` — 64.64 fixed point, exp2/log2/ln, maker-adverse rounding, error bounds | 04 §4 | M0 | 🔨 | Interim no_std deterministic transcendental helpers landed; replace with the final ≤2 ulp polynomial/integer kernel, then add the full MPFR-256 corpus gate |
