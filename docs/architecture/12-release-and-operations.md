...
- v(N+1) primary and exact paired v(N+2) recovery descriptors MUST be generated from the **queue-time artifact commitments** (the wasm/metadata hashes committed when the CODE/META proposal is queued — [09](09-execution-upgrades-and-rollout.md)), not from the post-enactment chain. The timelock is the build window; the primary is not eligible until both live-capable versions are covered.
