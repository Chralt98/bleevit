...
1. **Primary action — "Merge pairs → 100% recovery."** If the user holds pairable holdings (Accept+Reject branch-USDC pairs, or LONG+SHORT scalar sets within a branch), the screen leads with `merge`/`merge_scalar` — but **only a cross-branch pair recovers par**. `merge_scalar` on a same-branch set pays no USDC: it climbs the set to one same-branch branch-USDC, which recovers par only if the user also holds (or can acquire) the opposite branch, and 0.5 otherwise. The screen MUST show the recovery figure the user's *actual* holdings reach, never a headline 100 % that assumes the missing branch (SQ-171). The screen computes the user's maximal pairable amount across their positions and pre-fills it.
