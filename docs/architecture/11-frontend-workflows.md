...
**`nav()` view (rendered, at last).** The treasury screen renders every `NavView` component: liquid USDC at par, undisbursed stream remainders, obligations, in-flight XCM (marked 0, with copy explaining the conservative rule), VIT holdings (marked 0 in spendable NAV) — plus meter utilization (per-proposal/30 d/180 d) as gauges. **Reserve-haircut flag**: when the reserve-health trigger R is set ([07](07-oracle-and-disputes.md)/[08](08-treasury-and-economics.md) — e.g. a frozen USDC sovereign account), `nav()` carries the haircut flag; the FE MUST replace the headline NAV with the haircut presentation and a persistent banner *"reserve health degraded — NAV shown with haircut; split inflows halted (PB-RESERVE)"*. The FE never renders full backing while the flag is set.
