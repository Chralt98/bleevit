...
**Scope of the 100% slash (normative; SQ-287 resolution, 2026-07-20).** Rule 5's 100% applies to **screening-stage (T4) findings that are *verified*** — a verified constitution violation, or a verified false resource declaration. It does **not** reach a *decide-time* preimage mismatch, which terminates `Rejected(ConstitutionViolation)` under [05](./05-welfare-and-decision-engine.md) §1.3 but carries rule 2's **10%**. Two reasons, and they are the same reason twice. First, confiscation requires a verified culpable act (the T4 disposition taxonomy of [05](./05-welfare-and-decision-engine.md) §2.1): every residual, ambiguous or implementation-unverifiable static failure cancels refundably, and rule 2 already prices the preimage family at 10%. Second, and decisively, [09](./09-execution-upgrades-and-rollout.md) §7.3 **pins the preimage at qualification** precisely so the proposer cannot unnote it — the sabotage window is structurally empty. A preimage that has gone missing or mismatched by decide time is therefore an infrastructure or third-party failure, not proposer fault, and slashing it in full would punish the one party the pinning design already establishes is not responsible. The reason code stays `ConstitutionViolation` (it names *what* failed, not who is culpable); only the disposition is pinned here.
...
- **State-machine hooks** (transition numbers and pseudocode owned by [05](./05-welfare-and-decision-engine.md)): `Queued → Extended` (guardian force-rerun), `Trading/Extended → Extended` (window reset), then the ordinary decide transitions; [05](./05-welfare-and-decision-engine.md)'s T13-family rule "rerun re-enters Extended (3 days) then decides" covers both rerun kinds. **This includes the `δ + 1 pp` regime (SQ-188 resolution, 2026-07-20):** a force-rerun sets the same durable `Proposal.rerun` flag T13 sets, so the raised hurdle applies here exactly as it does to a `delay_once`-scheduled rerun, and it is consumed by the single `decide()` at the close of the fresh Extended window this call opens — the binding is pinned in [05](./05-welfare-and-decision-engine.md) §5.4. The "one guardian rerun of either kind, ever" bound above is enforced through that same flag.
...
- **Effect.** `ledger.set_frozen(true)` and `market.set_frozen(true)`: **all** ledger calls (split, merge, split_scalar, merge_scalar, transfer, redeem, redeem_scalar, redeem_void) and all market calls (buy, sell, crank_observe) error `Frozen` — both inflow and outflow, because during an unattributed drift either direction can be the exploit. The reconciliation crank, guardian calls, values referenda, and oracle/dispute calls remain live. Settlement calls are deliberately **not** frozen — a freeze must not strand a terminal vault transition.
- **Effect on live proposals (normative; amended 2026-07-20, SQ-98).** An active freeze force-rejects every live non-terminal proposal as **T20 `Rejected(ProcessHold)` with a par vault void** ([05](./05-welfare-and-decision-engine.md) §2.1; D-1) — *not* the `NotDecisionGrade` this bullet previously implied, and not scoped only to windows that overlap the freeze. Two corrections are load-bearing. First, the **reason code**: `NotDecisionGrade` forfeits 10 % of the intake bond to INSURANCE (§4(3)), which would fine proposers for an operational emergency they did not cause; `ProcessHold` refunds in full, which is the G-1 reading. Second, the **disposition**: the freeze halts `market.crank_observe`, so a proposal pushed into measurement would await a score the freeze itself prevents producing — a par void terminates cleanly instead, and D-1's ½ valuation is conservation-safe on every path ([03](./03-conditional-ledger.md) §6.4/§6.5). This disposition is identical whether the keeper's `tick` sweep or `decide()` observes the proposal first; before SQ-98 they diverged, so a race decided whether Accept-branch holders recovered ½ or nothing. G-1 is preserved, never inverted.
- **Sweep scope (normative; SQ-286 resolution, 2026-07-20).** The sweep covers **every** live non-terminal state, `Queued` and `FailedExecuted` included — an already-adopted mandate is force-rejected, not held to wait the freeze out. Rationale: force-rejection *is* the status quo (the payload does not dispatch either way), and the alternative — letting a mandate wait — would require extending its maturity and grace windows for the freeze's duration, machinery that exists only for the dead-man ([05](./05-welfare-and-decision-engine.md) §4.8) and not for a freeze. Without that extension a waiting mandate simply expires mid-freeze, which is the same terminal outcome reached later and with a worse bond disposition. [09](./09-execution-upgrades-and-rollout.md) §1.2(10)'s "no active PB-LEDGER-FREEZE" precondition on `execute()` is **not** made vacuous by this: the keeper's sweep is per-proposal and bounded, so between a freeze activating and the sweep reaching a given mandate there is a real window in which `execute()` must refuse, and §1.2(10) is what refuses it.
- **Precedence against the dead-man (normative; SQ-286 resolution, 2026-07-20).** When a freeze and a dead-man pause are active together, **the freeze wins**: the T20 sweep force-rejects, and the dead-man's "hold everything" no-op is not reached. The two regimes answer different failures — the dead-man is a *liveness* pause that preserves state until the chain recovers, while `PB-LEDGER-FREEZE` is a machine-checked *solvency* anomaly (the I-4 drift flag) in which the ledger's backing is in doubt. A stalled clock is not a reason to keep live proposals standing over a possibly-unbacked ledger, and the par void is the disposition that bounds the damage. Implementations therefore evaluate the freeze/stale-epoch sweep **before** the dead-man hold, which is the shipped ordering.
- **Precedence, and what it costs (normative).** The freeze test is evaluated **before** the decide-time static guards, so during a freeze a proposal that would otherwise terminate `Rejected(ConstitutionViolation)` (decide-time preimage mismatch) or `Rejected(ResourceConflict)` instead terminates `Rejected(ProcessHold)` with a full bond refund, forgoing the §4(5) slash. This is deliberate and is the price of convergence: evaluating the static guards first would keep `decide()` on the `reject_to_measurement` path for exactly those proposals and so **reintroduce** the tick/decide divergence the amendment exists to remove. It is also the conservative direction — a machine-checked solvency anomaly is precisely the state in which a static finding is least trustworthy, and confiscation requires a *verified* culpable act. The exposure is bounded: `PB-LEDGER-FREEZE` is not proposer-triggerable (it requires the I-4 drift flag), and the payload does not execute on either path. Note this was already race-dependent before SQ-98 — `tick` refunded while `decide()` slashed — so the amendment settles an existing ambiguity rather than opening a new one.
