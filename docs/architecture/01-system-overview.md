...
| `pallet-attestor` | Thin attestation registry ([06 §7](06-governance-and-guardians.md)): attestor set (≥ 3 members, guardian-track elected, 25k-VIT bonds), `AttestationRecord` lifecycle (`attest`/`challenge_attestation`, 72 h windows), 2-of-N quorum surfaced to `decide()` and the execution guard; storage/view shape to be frozen in [02](02-integration-contract.md) §7 at the B2 contract amendment (not yet present — the FE reads attestation status on the CODE/META execute path, so this row is queued with SQ-2/SQ-23) | constitution, values origins | [06](06-governance-and-guardians.md) |
