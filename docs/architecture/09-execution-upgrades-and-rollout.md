...
Writers, exhaustively: (a) the execution guard at `UpgradeAuthorized` (`spec_version` target, `pending_authorized_at`, the `URGENT_UPGRADE` flag), at applied-upgrade detection (clears pending fields) and at **relay-abort cleanup** (§2.1(7); clears the same pending fields when they still describe the aborted upgrade — occasion added 2026-07-20 with SQ-131, previously missing from this enumeration); (b) `ConstitutionalValues` via `constitution.set_release_channel` for manifest-pointer, `min_supported_version` and keyring-generation/revocation updates, including expedited descriptor-only releases ([12](./12-release-and-operations.md)). No other origin can write it. The FE in-app upgrade warning for pinned-release users binds to this key ([12](./12-release-and-operations.md)).

**`updated_at` (offset 108) is written by every writer (normative; SQ-133 resolution, 2026-07-20).** The field records the block of the **last write to the record**, not the last manifest repoint: writer (a) sets it on each of its three occasions (authorization, applied-upgrade detection, and the §2.1(7) abort cleanup) **on which it actually writes the record**, exactly as writer (b) does on a repoint. Where the guard's tolerant clear declines to write — because the stored channel no longer describes this upgrade, writer (b)'s newer value being authoritative — it leaves `updated_at` alone too, since it left the record alone. The per-writer parentheticals in the writers sentence above — and in [02](./02-integration-contract.md) §12 — enumerate the fields each writer sets *purposefully*; they give neither writer exclusive title to an offset, and `updated_at` is metadata about the record itself. Manifest freshness is carried by `manifest_txid`/`release_json_hash`, which writer (a) never touches; monitoring that needs repoint lag MUST derive it from the manifest pointer, never from offset 108.
...
- **Challenge window**: 72h *(normative value: [13](./13-parameters.md))* from record submission. A bonded challenge alleging non-reproducibility routes through the [07](./07-oracle-and-disputes.md) dispute game (deterministic recomputation is the natural terminal step: rebuild and compare hashes); a challenge **resolved against the attestation** slashes the signing attestors, revokes the record, and any queued proposal referencing it fails §1.2(5) at execute. **Polarity (normative; SQ-97 resolution, 2026-07-20):** the frozen [02 §7.5](./02-integration-contract.md) enum `ChallengeStatus` names the verdict from the *attestation's* side — `Upheld` means the **attestation** was upheld and the challenger forfeits; `Rejected` means the attestation was struck and its signers forfeit. Read "an upheld challenge" in any older prose as `ChallengeStatus::Rejected`. Only `Rejected` and `Open` withhold an attestation from quorum.
- **Gating**: record validity + elapsed window is a **queue-time precondition** (§1.1(3)); record continued existence is a dispatch-time check (§1.2(5)). The two are checked against **different state**: queue time evaluates the live registry (member floor, distinct active signers, closed windows); dispatch time evaluates only the committed records' revocation/challenge status ([06](./06-governance-and-guardians.md) §7, *What binds where*; SQ-97).
...
- **One home, and it is not the queue row or the event.** The anchor lives in its own storage item for the duration of the halt. It is **not** a field of `QueuedExecution` — §1.2(13) deletes that row on successful execute, i.e. before any migration can fail — and it is **not** carried on `UpgradeAuthorized`, whose shape is frozen at two fields by [02 §6](./02-integration-contract.md) and cannot gain one without a contract bump and joint sign-off. The anchor itself needs neither: it is guard-internal state read by operators and try-state, absent from the [02](./02-integration-contract.md) surface.

**Lifetime.** The anchor is written at application, survives the `PendingUpgrade` clear, and is cleared when the migration completes or a recovery image applies. The guard's try-state pairing is therefore an **implication, not an equality** — `PendingUpgrade.is_some() ⇒ anchor present`, with the anchor permitted to outlive `PendingUpgrade` exactly while the application latch or a `MigrationHalt` source is set. The superseded both-Some/both-None equality is what made the vacuity invisible: both items were killed together at the relay go-ahead callback, so the assertion passed while the anchor 09 depends on did not exist.
