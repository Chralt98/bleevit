...
7. **Chain-served history budget:** `RecentCohortSummaries` max-encodes to **1 + 32 × 158 = 5,057 B** (the one-byte SCALE compact collection-length prefix plus 32 measured `CohortSummary` values, whose `MaxEncodedLen` includes the 12-proposal hard maximum) + 8 TWAP checkpoints × 196 markets at the [04 §7](04-markets-and-pricing.md) spec shape (8 × (4 B `u32` block + 32 B two-limb cumulative) + 1 length byte = 289 B/market; the `TwapCheckpoints` series is implemented at exactly this shape — B10, 2026-07-17, measured and asserted in the runtime PoV suite) = **5,057 + 56,644 = 61,701 B ≈ 60.3 KiB** of always-served light-client state, within the ≤ 70 KiB D-6 layer-1 budget stated in [02](02-integration-contract.md).
