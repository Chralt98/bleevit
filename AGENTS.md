...
| `runtime/` | B1a + B4 residual + B6 | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`, genesis presets with the 08 §2.1 VIT allocation/vesting). B6 wired `ExecutionGuard` (index 62) with the full upgrade path e2e. B4 residual wires `InflowCaps` (index 63), Params-backed DOT/USDC `TraderRates`, and the ready `PhaseInflowCaps` adapter; the XCM transactor remains blocked on SQ-101. Open follow-ups: wire `pallet-epoch` (reserved index 61) + implement `FutarchyApi` from `runtime-api/` in `impl_runtime_apis!` |
