...
| `runtime/bleavit-runtime/` | B1a + B4 residual + B6: the real Cumulus parachain runtime — `construct_runtime!` over the Track-A + standard/system pallets (`ExecutionGuard` index 62, `InflowCaps` index 63), `BaseCallFilter = SafetyFilter`, live constitution-backed DOT/USDC XCM trader rates, the `DescriptorLeadTime` apply gate and full 09 §2 upgrade path, plus genesis presets carrying the 08 §2.1 VIT allocation/vesting; Wasm-buildable (`--features substrate-wasm-builder`) |
