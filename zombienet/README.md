...
| `05-coretime-renewal-under-dead-man` | 09 §4/§7.1 | XCM topology, relay-block stall measurement, liveness deltas, and the renewal call's **exemption-reachability form**: the dispatch must traverse the staged freeze into treasury logic and fail exactly `RenewalWindowClosed` (09 §4 exemption proven; a filter/freeze rejection fails the drill). Quotes/`ops.coretime` funding are deliberately unstageable from a chain spec or signed call | Full **staged-renewal form** (success + `CoretimeRenewalCalled`, the 09 §7.1 Phase-1 exit form): the quote-noting seam + the class-origin `fund_budget_line` path + `RenewalDispatch` wiring (re-owned to **B12** at the B10 close, PLAN SQ-245/SQ-246) — until staged, a run under a live dead-man proves **freeze traversal only** (exemption-reachability), never a renewal, so the suite gate carries that entry; dead-man **engagement** landed with B10 (see drill 04) |
| `06-pb-migration` | 09 §3.2/§7.1 | First execution (G1, 2026-07-18) proved the guardian machinery live: seven-seat membership read, usable-balance funding, `propose_action` + three `approve_action`s all execute against the real runtime | **SQ-264**: no `migrations.force_failure`/`retry` exists (the 09 §3.2 control-surface `[VERIFY]` is unresolved), so the `MigrationHalt` trigger can never go live in a drill and the fifth, dispatching approval correctly refuses with `TriggerInactive`. The playbook effect substrate landed with B1b (#104), so this control surface is again the drill's sole blocker |
...
NOTE(B7, updated G1 + post-B10 merge): `Epoch` (61) and `ExecutionGuard` (62)
are instantiated in the runtime, and B10 landed the dead-man detector and the
production XCM composition. The drill-blocking absences are now the 09 §3.2
migration-control surface (PLAN SQ-264), the B12-owned coretime renewal
staging (PLAN SQ-245/SQ-246), and drill 07's helper readiness probe (it
