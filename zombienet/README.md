...
| `03-keeper-loss` | 15 §4.7; 09 §7.1 | Chain-liveness delta runs before the dependency gate | Keeper node/cranks: B9; helper fails loudly `keeper node absent — gated on B9` |
...
| `05-coretime-renewal-under-dead-man` | 09 §4/§7.1 | XCM topology, relay-block stall measurement, and liveness deltas | A8/A11 freeze wiring; treasury B4 XCM dispatcher/quote funding; first native run: G1 |
...
| `07-xcm-reserve-transfer` | 15 §4.7; 09 §6.1 | v5 inputs and decoded source/destination event correlation are complete; bleavit/1 fails before AH debit | B4 executor/router/caps + Location-keyed USDC wiring; AH chain-origin inbound ClaimAsset dispatch must be verified |
