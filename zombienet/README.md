...
| `06-pb-migration` | 09 §3.2/§7.1 | First execution (G1, 2026-07-18) proved the guardian machinery live: seven-seat membership read, usable-balance funding, `propose_action` + three `approve_action`s all execute against the real runtime | **SQ-274**: no `migrations.force_failure`/`retry` exists (the 09 §3.2 control-surface `[VERIFY]` is unresolved), so the `MigrationHalt` trigger can never go live in a drill and the fifth, dispatching approval correctly refuses with `TriggerInactive`. The playbook effect substrate landed with B1b (#104), so this control surface is again the drill's sole blocker |
| `07-xcm-reserve-transfer` | 15 §4.7; 09 §6.1 | v5 inputs and decoded source/destination event correlation are complete; readiness is a genuine **capability probe** on the genesis-registered canonical Location-keyed USDC (SQ-101), not a version sentinel; the recovery `ClaimAsset` send is dispatched through the local preset's **sudo** so it carries Asset Hub's bare chain origin (a signed send would descend to the signer's account origin and could never match the AH-chain-keyed trap) | none — the stale `spec_version === 1` readiness sentinel is **retired** (2026-07-20): the B10 production XCM composition (`AssetTransactor`, `Barrier`, `IsReserve`, `IsTeleporter`, `OriginConverter`, sender) is present exactly when the canonical USDC is registered, which the helper now probes. First real run pending; AH sudo presence in the generated local preset is confirmed at first run |
...
(authority quote-noting, `RenewalDispatch`). The remaining drill-blocking
items are the 09 §3.2 migration-control surface (PLAN SQ-274, drill 06) and
drill 05's drill-side renewal staging; drill 07's stale `spec_version === 1`
readiness sentinel was **retired 2026-07-20** — readiness now probes the
genesis-registered canonical Location-keyed USDC (SQ-101), which is present
exactly when the B10 XCM composition is. Helpers intentionally fail with
